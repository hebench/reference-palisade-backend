...
std::vector<std::vector<std::int64_t>> MatMultRowBenchmark::prepareMatrix(const hebench::APIBridge::NativeDataBuffer &buffer,
...
std::vector<lbcrypto::Plaintext> MatMultRowBenchmark::encodeM0(const std::vector<std::vector<std::int64_t>> &data)
{
    ...
    for (size_t i = 0; i < vec_container_a.size(); i++)
        plainM0.push_back(m_p_context->context()->MakePackedPlaintext(vec_container_a[i]));
    return plainM0;
}
...
std::vector<lbcrypto::Plaintext> MatMultRowBenchmark::encodeM1(const std::vector<std::vector<std::int64_t>> &data)
{
    ...
    for (size_t i = 0; i < vec_container_b.size(); i++)
        plainM1.push_back(m_p_context->context()->MakePackedPlaintext(vec_container_b[i]));
