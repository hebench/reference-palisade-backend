...
MatMultValBenchmarkDescription::MatMultValBenchmarkDescription()
{
    ...
    m_descriptor.other    = MatMultValID;
    ...
    default_workload_params.add<std::uint64_t>(MatMultValBenchmarkDescription::DefaultScaleExponent, "ScaleBits");
                                                                             ...
                                                                             const hebench::APIBridge::WorkloadParams *p_params)
{
    ...
    return new MatMultValBenchmark(ex_engine, m_descriptor, *p_params);
}

void MatMultValBenchmarkDescription::destroyBenchmark(hebench::cpp::BaseBenchmark *p_bench)
{
    MatMultValBenchmark *p = dynamic_cast<MatMultValBenchmark *>(p_bench);
    if (p)
        delete p;
}
...
MatMultValBenchmark::MatMultValBenchmark(PalisadeEngine &engine,
    ...
    m_w_params(bench_params)
{
    ...
    std::size_t scale_bits = m_w_params.get<std::uint64_t>(MatMultValBenchmarkDescription::Index_ScaleExponent);
...
std::vector<std::vector<double>> MatMultValBenchmark::prepareMatrix(const hebench::APIBridge::NativeDataBuffer &buffer,
...
std::vector<lbcrypto::Plaintext> MatMultValBenchmark::encodeMatrix(const std::vector<std::vector<double>> &data)
{
    ...
    for (size_t i = 0; i < data.size(); ++i)
    {
        if (data[i].size() != num_cols)
            throw hebench::cpp::HEBenchError(HEBERROR_MSG_CLASS("Non-uniform number of columns found in matrix."),
                                             HEBENCH_ECODE_INVALID_ARGS);
        retval[i] = m_p_context->context()->MakeCKKSPackedPlaintext(data[i]);
    } // end for
    return retval;
}

std::vector<lbcrypto::Plaintext> MatMultValBenchmark::encodeM0(const std::vector<std::vector<double>> &data)
{
...
std::vector<lbcrypto::Plaintext> MatMultValBenchmark::encodeM1(const std::vector<std::vector<double>> &data)
{
...
std::vector<lbcrypto::Ciphertext<lbcrypto::DCRTPoly>> MatMultValBenchmark::encryptMatrix(const std::vector<lbcrypto::Plaintext> &plain)
{
...
MatMultValBenchmark::doMatMultVal(const std::vector<lbcrypto::Ciphertext<lbcrypto::DCRTPoly>> &M0,
...
void MatMultValBenchmark::decode(hebench::APIBridge::Handle h_encoded_data, hebench::APIBridge::DataPackCollection *p_native)
{
    ...
    const std::vector<std::vector<lbcrypto::Plaintext>> &encoded_result =
        this->getEngine().template retrieveFromHandle<std::vector<std::vector<lbcrypto::Plaintext>>>(h_encoded_data, MatMultValBenchmark::tagEncodedResult);
                ...
                for (std::size_t row_i = 0; row_i < m_w_params.rows_M0(); ++row_i)
                {
                    for (std::size_t col_i = 0; col_i < m_w_params.cols_M1(); ++col_i)
                    {
                        std::vector<double> decoded =
                            encoded_result[row_i][col_i]->GetRealPackedValue();
...
hebench::APIBridge::Handle MatMultValBenchmark::decrypt(hebench::APIBridge::Handle h_encrypted_data)
{
    ...
    const std::vector<std::vector<lbcrypto::Ciphertext<lbcrypto::DCRTPoly>>> &encrypted_result =
        this->getEngine().template retrieveFromHandle<std::vector<std::vector<lbcrypto::Ciphertext<lbcrypto::DCRTPoly>>>>(h_encrypted_data, MatMultValBenchmark::tagEncryptedResult);
                                                        ...
                                                        std::uint64_t indexers_count)
{
    ...
    // validate indexers
    for (std::size_t param_i = 0; param_i < MatMultValBenchmarkDescription::NumOpParams; ++param_i)
    {
